//g++ -std=c++11 assignment4.cpp -o assignment4
//./assignment4
...
T DoublyLinkedList<T>::get(const unsigned int index) const {
  ...
  if (!this->first){
    ...
    cout << "Cannot apply negative index." << endl;
    throw 1;
  } else if (signedIndex > size) {
    cout << "Index is larger than list size." << endl;
    throw 1;
  }
  else {
    while (signedIndex > counter) {
      current = current->forward;
      counter++;
    }
  }
...
void DoublyLinkedList<T>::insert(const unsigned int index, const T& value) {
  ...
  current = this->first;
  // Insert at beginning if list doesn't exist
  if (size == 0){
  ...
  } else if (signedIndex == 0 && size > 0){
    Node<T>* newNode = new Node<T>();
    newNode->data = value;
    newNode->forward = current;
...
// If list is empty
  if (size == 0){
    ...
    return;
// If index is larger than list
  } else if (index > size - 1){
    cout << "Index is larger than list size." << endl;
    return;
// If list is only one node and should be removed
  } else if (index == 0 && size == 1){
    this->first = nullptr;
    this->last = nullptr;
    delete current;
// If list is more than one node and first node should be removed
  } else if (index == 0){
    this->first = current->forward;
    current->forward->backward = nullptr;
    delete current;
// If last node should be removed
  } else if (index == size - 1){
    while (current->forward != nullptr){
      current = current->forward;
    }
    this->last = current->backward;
    current->backward->forward = nullptr;
    delete current;
// Remove all other nodes
      ...
      while (index >= counter){
        ...
        counter++;
        if (index - 1 == counter){
          Node<T>* tempNode = current->forward;
          current->forward = tempNode->forward;
...
void DoublyLinkedList<T>::removeAllInstances(const T& value){
  ...
  if (!this->first){
    ...
    return;
  // If list has one node that should be removed
  } else if (this->first->data == value && current->forward == nullptr){
    this->first = nullptr;
    this->last = nullptr;
    delete current;
  } 
    ...
    // If the first node needs to be deleted and there are multiple nodes in the list
      if (this->first->data == value){
        Node<T>* tempNode = current->backward;
        this->first = current;
        current->backward = nullptr;
        delete tempNode;
    // If the last node should be removed
      } else if (this->last->data == value && current->forward == nullptr){
        this->last = current->backward;
        current->backward->forward = nullptr;
        delete current;
