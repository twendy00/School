...
T SinglyLinkedList<T>::getFifthElement() const {
  ...
  while(current->link != this->last && position < 5){
...
void SinglyLinkedList<T>::swapFourthAndFifthElement(){
  ...
  Node<T>* fourth = nullptr;
  Node<T>* fifth = nullptr;
  Node<T>* sixth = nullptr;
  int position = 1;
  ...
  while (current->link != nullptr){
    ...
    third = fourth->link; // connects 3rd node to 5th node
    fourth = fifth->link; // connects 4th node to 6th node
    fifth = fourth->link; // connects 5th node to 4th node
